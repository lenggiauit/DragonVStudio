...
const Conversationer: React.FC<Props> = ({
  hubConnection,
  data,
  selectedConversationEvent,
  currentUser,
  selectedConversation,
}) => {
  const { locale } = useAppContext()

  const [lastcurrentMessage, setlastcurrentMessage] = useState<string>(
    data?.lastMessage
  )

  const [isReceivedMessageButNotActive, setIsReceivedMessageButNotActive] =
      ...
      hubConnection.on('onConversationReceivedMessage', (receivedMessage) => {
        try {
          ...
          if (convMessage) {
            ...
            if (
              convMessage.conversationId == data?.id &&
  ...
  if (data) {
    ...
    return (
      <>
        <div
          className={
            'conversationer-item-container ' +
            (data.id == selectedConversation?.id ? 'active-conversation' : '') +
            (isReceivedMessageButNotActive ? 'receiving-message' : '')
          }
          onClick={() => {
            selectedConversationEvent(data)
          }}
        >
          <div className='conversationer-item-body-container p-2'>
            <div className='row m-0'>
              ...
              <div className='col-md-8 text-left m-0 pr-0 pl-0 align-self-center'>
                <div>{title}</div>
                <div>
                  {lastcurrentMessage != null && lastcurrentMessage.length > 20
                    ? lastcurrentMessage.substring(0, 20)
                    : lastcurrentMessage}
                </div>
              </div>
            </div>
