...
const InviteMember: React.FC<Props> = ({
  isShow,
  conv,
  onClose,
  onSelected,
  ...
  const [messengerSearchByKeyword, messengerSearchByKeywordStatus] =
  ...
  > = (e) => {
    ...
    if (searchkeys.length >= 2 && e.key !== 'Backspace') {
      messengerSearchByKeyword({
        payload: {
          keyword: searchkeys,
          currentIds: conv.conversationers.map((c) => c.id),
        },
      })
    }
  }
  ...
  useEffect(() => {
    if (
      messengerSearchByKeywordStatus.isSuccess &&
      messengerSearchByKeywordStatus.data.resultCode == ResultCode.Success
    ) {
