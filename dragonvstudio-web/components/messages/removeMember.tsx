...
const RemoveMember: React.FC<Props> = ({
  conv,
  currentUser,
  onClose,
  onSelected,
  ...
  const [filterMessengers, setfilterMessengers] = useState<Messenger[]>(
    conv.conversationers.filter((m) => m.id != currentUser.id)
  )
  const [seletedMessengers, setSeletedMessengers] = useState<Messenger[]>([])

  // handle searching user
  const handleSeachingMessenger: React.KeyboardEventHandler<
    HTMLInputElement
  > = (e) => {
    let searchkeys = e.currentTarget.value
    if (searchkeys.length >= 1 && e.key !== 'Backspace') {
      let list = filterMessengers.filter(
        (m) =>
          m.fullName?.toLowerCase().indexOf(searchkeys.toLowerCase()) != -1 ||
          m.name?.toLowerCase().indexOf(searchkeys.toLowerCase())
      )
  ...
  const onSelectedMessenger: any = (messenger: Messenger) => {
    ...
    if (listselectedMessengers.filter((m) => m.id == messenger.id).length > 0) {
      listselectedMessengers = listselectedMessengers.filter(
        (m) => m.id != messenger.id
      )
    } else {
      if (
        conv.conversationers.filter((m) => m.id != currentUser.id).length >
        listselectedMessengers.length + 1
      ) {
          ...
          <div className='modal-content'>
            <div className='modal-header'>
              <h5 className='modal-title' id='removeModalLabel'>
                <Translation tid='InviteToConversation' />
              </h5>
              <button
                type='button'
                className='close'
            ...
            <div className='modal-body pb-0 pt-2'>
              <div className='input-group input-group-search mb-2'>
                <input
                  type='text'
                  ref={txtSearchRef}
                  className='form-control border-1 form-control-sm rounded-pill'
                  onKeyDown={handleSeachingMessenger}
                    ...
                    {filterMessengers
                      ...
                      .map((c, i) => (
                        <div
                          key={i + v4().toString()}
                          className={
                            'container m-0 messenger-item ' +
                            (seletedMessengers.filter((m) => m.id == c.id)
                              .length > 0
                              ? 'selected'
                              : '')
                          }
                          onClick={() => onSelectedMessenger(c)}
                        >
                          <div className='row'>
                            <div className='col-md-2 m-0 pl-0 conversation-avatars-container'>
                              <img
                                key={'avatar-' + i + v4().toString()}
                                src={c.avatar ?? '/assets/images/Avatar.png'}
                                className='rounded-circle conversation-avatars'
                              />
                            </div>
