...
const ConversationDetail: React.FC<Props> = ({
  ...
  const [listReceivedMessage, setListReceivedMessage] = useState<
    ConversationMessage[]
  >([])
  const [getMessagesByConversation, getMessagesByConversationStatus] =
    useGetMessagesByConversationMutation()
  const [deleteConversation, deleteConversationStatus] =
    useDeleteConversationMutation()
  const [inviteToConversation, inviteToConversationStatus] =
    useInviteToConversationMutation()
  const [removeFromConversation, removeFromConversationStatus] =
    useRemoveFromConversationMutation()
  ...
  const onSelectedRemoveMember: any = (ms: Messenger[]) => {
    ...
    if (currentConversation) {
      let removeConv: Conversation = {
        ...
        title: currentConversation.title,
        createdBy: currentConversation.createdBy,
        conversationers: currentConversation.conversationers.filter(
          (u) => !ms.includes(u)
        ),
        createdDate: currentConversation.createdBy,
        lastMessage: currentConversation.lastMessage,
  ...
  const onDeleteConversation: React.MouseEventHandler<HTMLAnchorElement> = (
    e
  ) => {
    e.preventDefault()
    showConfirmModal({
  ...
  if (currentConversation) {
    ...
    if (title == null || title.trim().length == 0) {
      title = currentConversation.conversationers
      ...
      <>
        {showInviteModal === true && (
          <InviteMember
            ...
            onClose={onCloseInviteMember}
            onSelected={onSelectedInviteMember}
          />
        )}
        ...
        {showRemoveModal === true && (
          <RemoveMember
            ...
            onSelected={onSelectedRemoveMember}
          />
        )}
        <div className='conversation-detail-container mt-2'>
          <div className='message-header-container p-2'>
            <div
              className='row m-0'
              key={'message-header-container-' + v4().toString()}
            >
              <div className='col-md-10 m-0 pl-0 '>
                <div className='row m-0 p-0'>
                  <div className='col-md-2 m-0 pl-0 '>
                    <div className='conversation-avatars-container'>
                      {currentConversation.conversationers
                        ...
                        .map((c, i) => (
                          <img
                            key={'avatar-detail-' + i + v4().toString()}
                            src={c.avatar ?? '/assets/images/Avatar.png'}
                            className='rounded-circle multiple conversation-avatars'
                          />
                        ))}
                    </div>
                  </div>
                  <div className='col-md-10 m-0 pl-0 '>
                    ...
                    <div>
                      {new Date(
                        currentConversation?.lastMessageDate
              ...
              <div className='col-md-2 text-left m-0 pr-0 pl-0 text-right'>
                <a
                  className='btn btn-link p-0 m-1 d-inline align-baseline '
                  href='#'
              ...
              {getMessagesByConversationStatus.isSuccess && (
                <>
                  <Scrollbars ref={scrollbarsRef}>
                    {getMessagesByConversationStatus.data &&
                      getMessagesByConversationStatus.data.resource.map(
                        (item, index) => (
                          <ConversationMessageItem
                            key={'message-' + index + v4().toString()}
                            hubConnection={hubConnection}
                            message={item}
                            currentUser={currentUser}
                            user={
                              currentConversation.conversationers.filter(
                                (c) => c.id == item.userId
                              )[0]
                            }
                          />
                      ...
                      {currentConversation.conversationers
                        ...
                        .map((c, i) => (
                          <img
                            key={'avatar-detail-' + i + v4().toString()}
                            src={c.avatar ?? '/assets/images/Avatar.png'}
                            className='rounded-circle typing-avatar'
                          />
                        ))}
                    </div>
                    <div className='d-inline'>
                      <Translation tid='isTyping' />
                    </div>
                  </div>
                </>
              )}
