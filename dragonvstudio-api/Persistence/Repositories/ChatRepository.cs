...
namespace DragonVStudio.API.Persistence.Repositories
{
    public class ChatRepository : BaseRepository, IChatRepository
    {
        ...
        public ChatRepository(DragonVContext context, ILogger<ChatRepository> logger) : base(context)
        {
            _logger = logger;
        }
        ...
        public async Task<List<Conversation>> GetConversationListByUser(Guid userId, BaseRequest<GetConversationListRequest> request, AppSettings appSettings)
        {
            try
            {
                // init chat with Dragon V Studio
                ...
                if(list.Count == 0)
                {
                    var naUser = _context.User.Where(u => u.Email.Equals("DragonVStudio.vn@gmail.com")).FirstOrDefault();
                    if (naUser != null)
                    {
